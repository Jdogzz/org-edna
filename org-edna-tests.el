...
(ert-deftest org-edna-doc-test/activate-with-no-state ()
  (org-edna-doc-test-setup "72dd4f0d-6a99-4214-92eb-0d2fcbe52c72"
    ;; Property should only change when we switch to DONE (and not "")
    (org-todo "DONE")
    (should (string-equal (org-entry-get nil "TEST") "1"))
...
(ert-deftest org-edna-doc-test/activate-with-no-state ()
  (org-edna-with-test-heading "72dd4f0d-6a99-4214-92eb-0d2fcbe52c72"
    (let ((pom (point-marker)))
      ;; Property should only change when we switch to DONE (and not "")
      (org-edna-test-change-todo-state pom "DONE")
      (should (string-equal (org-entry-get pom "TEST") "1"))

      (org-edna-test-change-todo-state pom "TODO")
      (should (string-equal (org-entry-get pom "TEST") "1"))

      (org-edna-test-change-todo-state pom "")
      (should (string-equal (org-entry-get pom "TEST") "1")))))

(ert-deftest org-edna-doc-test/activate-from-no-state ()
  (org-edna-with-test-heading "1d8fe565-db0a-4478-bd8f-f8aed87c458e"
    (let ((pom (point-marker)))
      ;; Mark as done; nothing should happen
      (let* ((org-edna-from-todo-states 'todo))
        (org-edna-test-change-todo-state pom "DONE")
        (should (string-equal (org-entry-get pom "TEST") "0"))
        (org-edna-test-change-todo-state pom ""))

      ;; Now something should happen
      (let* ((org-edna-from-todo-states 'not-done))
        (org-edna-test-change-todo-state pom "DONE")
        (should (string-equal (org-entry-get pom "TEST") "1"))
        (org-edna-test-change-todo-state pom "")))))

(ert-deftest org-edna-doc-test/trigger-property-change ()
  (org-edna-with-test-heading "bf803c62-c83c-41e6-99e2-609c44f5205d"
    (let ((pom (point-marker)))
      (org-edna-test-mark-done pom)
      (should (string-equal (org-entry-get pom "REPEAT") "1"))
      (should (org-edna-test-compare-todos pom "TODO" "First Repeat"))

      (org-edna-test-mark-done pom)
      (should (string-equal (org-entry-get pom "REPEAT") "2"))
      (should (org-edna-test-compare-todos pom "TODO" "Second Repeat"))

      (org-edna-test-mark-done pom)
      (should (string-equal (org-entry-get pom "REPEAT") "2"))
      (should (org-edna-test-compare-todos pom "DONE" "Final Repeat")))))

(ert-deftest org-edna-doc-test/trigger-property-change-with-schedule ()
  (org-edna-with-test-heading "cd3e7fb2-c8ab-4eac-bb3b-b444fdd88bb4"
    (let ((pom (point-marker)))
      (org-edna-test-mark-done pom)
      (should (string-equal (org-entry-get pom "REPEAT") "1"))
      (should (org-edna-test-compare-todos pom "TODO" "First Repeat"))
      (should (string-equal (org-entry-get nil "SCHEDULED") "<2000-01-16 Sun>"))

      (org-edna-test-mark-done pom)
      (should (string-equal (org-entry-get pom "REPEAT") "2"))
      (should (org-edna-test-compare-todos pom "TODO" "Second Repeat"))
      (should (string-equal (org-entry-get nil "SCHEDULED") "<2000-01-17 Mon>"))

      (org-edna-test-mark-done pom)
      (should (string-equal (org-entry-get pom "REPEAT") "2"))
      (should (org-edna-test-compare-todos pom "DONE" "Second Repeat"))
      (should (string-equal (org-entry-get nil "SCHEDULED") "<2000-01-17 Mon>")))))
