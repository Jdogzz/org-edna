    ...
    (cond
     ...
     (t
      ...
      (let* ((case-fold-search t)
             ...
             (final-time (encode-time (swap-nil-times parsed-time)))
             (new-ts (format-time-string (if have-time "%F %R" "%F") final-time)))
        (org--deadline-or-schedule nil type new-ts))))))

(defun swap-nil-times (temptime)
  (setcar (nthcdr 1 temptime) (funcall (lambda (e) (or e 0)) (nth 1 temptime)))
  (setcar (nthcdr 2 temptime) (funcall (lambda (e) (or e 0)) (nth 2 temptime)))
  temptime
  )
